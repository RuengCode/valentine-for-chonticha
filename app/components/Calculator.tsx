...
const showAlertฺ = (
  ...
  alertDiv.className = `alert alert-${type} m-4 opacity-0 transition-opacity duration-500 ease-in-out`;
  alertDiv.innerHTML = `
    ${iconSvg[type]}
    <span>${message}</span>
  `;

  // Gradually show the alert
  setTimeout(() => {
    alertDiv.classList.add('opacity-100');
  }, 10);

  // Gradually hide the alert after 3 seconds
  setTimeout(() => {
